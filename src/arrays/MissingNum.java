...
public class MissingNum {
	...
	public static void main(String[] args) {
		final Integer[] arr = { 389, 299, 65, 518, 361, 103, 342, 406, 24, 79, 192, 181, 178, 205, 38, 298, 218, 143, 446, 324,
		...
		ArrayList<Integer> data = (ArrayList<Integer>) Stream.of(arr).collect(Collectors.toList());
		List<Integer> val = repeatedNumber(data);
		val.forEach(_data -> System.out.println(_data));
		
	}

	public static ArrayList<Integer> repeatedNumber(final List<Integer> arrList) {
		...
		for (Integer num : arrList) {
			...
			sum = sum.add(new java.math.BigInteger(String.valueOf(num)));
		}
		sum = sum.add(new BigInteger(String.valueOf(-1 * A)));
		long length = arrList.size();
		Long totalSum = null;
		if(length % 2 == 0) {
			totalSum = ((length / 2) * (length + 1));
		} else {
			totalSum = length  * ((length + 1) / 2 );
		}
		Long B = totalSum - sum.longValue();
